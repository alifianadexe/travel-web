...
class CART extends CI_Controller {
    ...
    public function removeFromCart() {
        ...
        if ($succ) $message = '<div class="alert alert-success" style="margin-top:10px" role="alert"> Item removed from your order. </div>';
        else $message = '<div class="alert alert-danger" style="margin-top:10px" role="alert"> Failed to remove item from your order. </div>';       
    ...
    public function checkout() {
        ...
        if ($count == 0) {
            $message = '<div class="alert alert-danger" style="margin-top:10px" role="alert"> You have No products in your order. </div>'; 
            $this->session->set_flashdata('msg', $message);
            redirect('user/your_cart');
        } else {
            $this->cart_model->buyCart($cartid);
            $message = '<div class="alert alert-success" style="margin-top:10px" role="alert"> You have purchased your order. </div>'; 
            $this->session->set_flashdata('msg', $message);
            redirect('user/your_cart');
        }
    }
